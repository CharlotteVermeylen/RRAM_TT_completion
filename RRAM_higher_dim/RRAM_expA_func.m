% This script generates the experiment in the manuscript with the
% RRAM and an exponential function evaluation (section 4.3.2).
...
for i1=1:n
    for i2=1:n
        for i3=1:n
            for i4=1:n
                A(i1,i2,i3,i4) = exp(-norm([x(i1);x(i2);x(i3);x(i4)]));
            end
        end
    end
end

sizeOmega = 0.1*prod(n);
sizeGamma = 100;
rng(1)
...
disp(length(A_Omega)/prod(n))

rx = [1, ones(1,d-1), 1];
X0 = TTeMPS_randn(rx, n);

%%
opts1 = struct('maxiter',15,'gradtol',10^(-8),'reltol',10^(-8),'tol',10^(-8));
opts1.maxiter_final = opts1.maxiter;
opts1.maxrank = 5;
opts2 = struct('kmax',15,'eps_gamma',1,...
    'r_max',5*ones(1,d-1),'s_max',5*ones(1,d-1),'Delta',0.8);
...
%%
[X2,cost_tmp2,test_tmp2,stats2,ranks2] = RRAM_TT_completion_def_gen(...
