function [X,cost,test,stats,ranks] = completion_rankincrease( method, A_Omega, Omega, A_Omega_C, Omega_C, A_Gamma, Gamma, X0, opts )
    % ranks is an extra output argument added by Charlotte
    ...
    %test = []; Commented by Charlotte:
    test = norm(X0(Gamma) - A_Gamma)/norm(A_Gamma); % Added by Charlotte
    stats.time = 0; % Charlotte
    test = [test,norm(X(Gamma) - A_Gamma)/norm(A_Gamma)]; % Added by Charlotte
    stats.time = [stats.time,toc]; % Added by Charlotte
    ranks = X.rank; % Added by Charlotte
...
%                 stats.time = [stats.time, stats_tmp.time]; % Commented by Charlotte
                ranks = [ranks;X.rank]; % Added by Charlotte
            end
